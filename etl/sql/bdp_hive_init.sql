-- 每日从bdp导出的tsv，导入中台的hive

create database yc_bit

CREATE EXTERNAL TABLE `yc_bit`.`fo_service_order`(
...
STORED AS parquet
LOCATION
  'hdfs://hdp25/user/hive/warehouse/yc_bit.db/fo_service_order'




CREATE EXTERNAL TABLE `yc_bit`.`fo_service_order_ext`(
  `service_order_id` bigint,
  `operator_id` int,
  `user_type` tinyint,
  `sms` string,
...
STORED AS parquet
LOCATION
  'hdfs://hdp25/user/hive/warehouse/yc_bit.db/fo_service_order_ext'
...
CREATE EXTERNAL TABLE `yc_bit`.`ods_service_order`(
  `service_order_id` bigint,
  `product_type_id` int,
  `fixed_product_id` int,
  `city` string,
...
STORED AS parquet
LOCATION
  'hdfs://hdp25/user/hive/warehouse/yc_bit.db/ods_service_order'



CREATE EXTERNAL TABLE `yc_bit`.`ods_service_order_charge`(
  `service_order_id` bigint,
...
STORED AS parquet
LOCATION
  'hdfs://hdp25/user/hive/warehouse/yc_bit.db/ods_service_order_charge'
